...
std::function<int(Robot&, TKobukiData)> Robot::do_nothing_robot=[](Robot& self, TKobukiData data){return 0;};
std::function<int(Robot&, LaserMeasurement)> Robot::do_nothing_laser=[](Robot& self, LaserMeasurement data){return 0;};
...
Robot::Robot(std::string ipaddressLaser,int laserportRobot, int laserportMe,std::function<int(Robot&, LaserMeasurement)> &lascallback,std::string ipaddressRobot,int robotportRobot, int robotportMe,std::function<int(Robot&, TKobukiData)> &robcallback)
{
    ...
    doneFuture=done_promise.get_future();
}
...
void Robot::robotprocess()
{
    ...
    while(1)
    {
        if(doneFuture.wait_for(std::chrono::seconds(0))==std::future_status::ready)
            break;
        ...
        if(returnval==0)
        {
            ...
            std::async(std::launch::async, [this](TKobukiData sensdata) { robot_callback(*this, sensdata); },sens);
            std::call_once(flag_robot, [this](){ 
                std::lock_guard<std::mutex> l{initStateMutex};
                initState--;
                initStateCV.notify_all();
            });
...
void Robot::setRotationSpeed(double radpersec) //left
{
 ...
 const CKobuki& Robot::getKobuki()
 {
    return robot;
 }

const vector<TKobukiCoordinates>& Robot::getPath()
 {
    return robot.getKobukiPath();
 }

 TKobukiCoordinates Robot::getLastCoordinates()
 {
    return robot.getLastCoordinates();
 }

 bool Robot::waitConnection(std::chrono::seconds timeout)
 {
    std::unique_lock<std::mutex> lock{initStateMutex};
    return initStateCV.wait_for(lock, timeout, [this]() { return this->initState <= 0;});
 }
...
void Robot::laserprocess()
{
    ...
    while(1)
    {
        ...
        std::async(std::launch::async, [this](LaserMeasurement sensdata) { laser_callback(*this, sensdata); },measure);
        ///ako som vravel,toto vas nemusi zaujimat
        std::call_once(flag_laser, [this](){ 
                std::lock_guard<std::mutex> l{initStateMutex};
                initState--;
                initStateCV.notify_all();
            });
