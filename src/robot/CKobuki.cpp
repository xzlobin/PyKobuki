...
void CKobuki::kobukiDataPostprocess(const TKobukiData &data)
{
    static const auto DIFF_MAX_LR  { valued_max(data.EncoderLeft) };
    static const auto DIFF_MAX_TIME{ valued_max(data.timestamp)   };
    
    static auto prev_enc_L = static_cast<long long>(data.EncoderLeft);
    static auto prev_enc_R = static_cast<long long>(data.EncoderRight);
    static auto prev_time  = static_cast<unsigned long long>(data.timestamp);



    if (kobuki_estimated_path.empty()) 
    {
        kobuki_estimated_path.push_back({
            0,        // .x = 
            0,        // .y = 
            0,        // .phi = 
            prev_time // .time = 
    ...
    auto trueDifference = [](const auto enc, const auto enc_prev) { 
        auto ddiff = 2*(enc - enc_prev);
        if ( -ddiff >= DIFF_MAX_LR )
        {
            return (enc - enc_prev) + DIFF_MAX_LR;
        }
        else if ( ddiff >= DIFF_MAX_LR )
        {
            return (enc - enc_prev) - DIFF_MAX_LR;
        }
        return enc - enc_prev;
    ...
    auto traveled_L =  trueDifference(data.EncoderLeft, prev_enc_L);
    auto traveled_R =  trueDifference(data.EncoderRight, prev_enc_R);

    auto traveled_diff = traveled_R - traveled_L;
    auto angle_diff = tickToMeter * (traveled_diff)/b;


    auto traveled_center = 0.5L * tickToMeter * (traveled_R + traveled_L);
    auto angle_prev = kobuki_estimated_path.back().phi;
    auto angle = angle_prev + angle_diff;

    kobuki_estimated_path.push_back({
        kobuki_estimated_path.back().x + traveled_center * std::cos(angle),  // .x =
        kobuki_estimated_path.back().y + traveled_center * std::sin(angle),  // .y =
        angle,                                                                                                      // .phi = 
        data.timestamp - prev_time >= 0 ? data.timestamp - prev_time : (DIFF_MAX_TIME - prev_time) + data.timestamp // .time = 
    });

    prev_enc_L = data.EncoderLeft;
    prev_enc_R = data.EncoderRight;
