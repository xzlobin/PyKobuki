...
typedef struct{
    long double x; long double y; long double phi; unsigned long long time;
} TKobukiCoordinates;
...
public:
    ...
    {
        int fillResult = parseKobukiMessage(output,message);

        // postprocessing data to find robot path and etc.
        if (fillResult == 0)
        {
            kobukiDataPostprocess(output);
        }

        return fillResult;
    }

    const vector<TKobukiCoordinates>& getKobukiPath()
    {
        return kobuki_estimated_path;
    }
    ...
    TKobukiCoordinates getLastCoordinates()
    {
        ...
        {
            return kobuki_estimated_path.back();
        }
        else{
            return {0, 0, 0, 0};
        }
...
//
    ...
    //added functionality
    std::vector<TKobukiCoordinates> kobuki_estimated_path;
    
    void kobukiDataPostprocess(const TKobukiData &data);


    template<typename T>
    constexpr T valued_max(const T& v)
    {
