...
public:
    ~Robot();
    Robot(std::string ipaddressLaser="127.0.0.1",int laserportRobot=52999, int laserportMe=5299,std::function<int(Robot&, LaserMeasurement)> &lascallback=do_nothing_laser,std::string ipaddressRobot="127.0.0.1",int robotportRobot=53000, int robotportMe=5300,std::function<int(Robot&, TKobukiData)> &robcallback=do_nothing_robot);


    //default functions.. please do not rewrite.. make your own callback
    static  std::function<int(Robot&, TKobukiData)> do_nothing_robot;
    static std::function<int(Robot&, LaserMeasurement)> do_nothing_laser;


    void robotStart();

    void setLaserParameters(std::string ipaddress,int laserportRobot, int laserportMe,std::function<int(Robot&, LaserMeasurement)> callback )
    {
    ...
    void setRobotParameters(std::string ipaddress,int robotportRobot, int robotportMe,std::function<int(Robot&, TKobukiData)> callback )
    {
    ...
    const CKobuki& getKobuki();
    const vector<TKobukiCoordinates>& getPath();
    TKobukiCoordinates getLastCoordinates();
    bool waitConnection(std::chrono::seconds timeout = std::chrono::seconds(60));

private:
    std::promise<void> done_promise;
    std::shared_future<void> doneFuture;
    
    std::mutex initStateMutex;
    std::condition_variable initStateCV;
    int initState = 0;
    ...
    std::function<int(Robot&, LaserMeasurement)> laser_callback=nullptr;
    ...
    void robotprocess();
    std::function<int(Robot&, TKobukiData)> robot_callback=nullptr;
